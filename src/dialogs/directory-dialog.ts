...
declare global {
	...
	interface HTMLElementTagNameMap {
		'directory-dialog': DirectoryDialog
	}
}

@customElement({name: 'directory-dialog', inject: true})
@withStyles()
export class DirectoryDialog extends LitElement {
	@state() open = false
			...
			switch (this.type) {
				case 'Create':
					...
					if (!ok) {
						...
						fs.reload()
						fs.enterDirectory(this.#new.name)
					}
					break
...
// export const dialogDirectoryDialog = new DialogDirectoryDialog();
